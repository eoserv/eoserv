...
template <> void Hook_Call::SetArg<asDWORD>(ScriptContext *ctx, int argc, asDWORD arg)  { printf("A %s [%i = %i]\n", typeid(arg).name(), argc, arg); ctx->as->SetArgDWord(argc, arg);   }
...
template <> void Hook_Call::SetArg<asQWORD>(ScriptContext *ctx, int argc, asQWORD arg)  { printf("A %s [%i = %i]\n", typeid(arg).name(), argc, arg); ctx->as->SetArgQWord(argc, arg);   }
template <> void Hook_Call::SetArg<float>(ScriptContext *ctx, int argc, float arg)      { printf("A %s [%i = %f]\n", typeid(arg).name(), argc, arg); ctx->as->SetArgFloat(argc, arg);   }
template <> void Hook_Call::SetArg<double>(ScriptContext *ctx, int argc, double arg)    { printf("A %s [%i = %g]\n", typeid(arg).name(), argc, arg); ctx->as->SetArgDouble(argc, arg);  }
